...
post '/create' do
  ...
  # 投稿を新規作成
  query = 'INSERT INTO posts (user_id,title, description, image_path,created_at,updated_at) VALUES (?,?,?,?,CURRENT_TIMESTAMP(),CURRENT_TIMESTAMP())'
  client.xquery(query, session[:user_id], params[:title], params[:description], "/img/#{@filename}")
